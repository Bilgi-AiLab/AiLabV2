    ...
    if request.method == 'POST':
        ...
        if algorithm.lower() == 'frequency':
            if selected_corpus_type == 'all_corpus':
                outputs = frequency(corpus, n_most, True)
            else:
                outputs = frequencySep(corpus, n_most, True)
            print("algoritmadan donen sonuc: ",outputs)

        elif algorithm.lower() == "n-gram":
            if selected_corpus_type == 'all_corpus':
                outputs = bigramsAll(corpus, n_word, n_most, True)
            else:
                outputs = bigrams(corpus, n_word, n_most, True)

        elif algorithm.lower() == "wordcloud":
            if selected_corpus_type == 'all_corpus':
                outputs = wordCloudAll(corpus, True)
            else:
                outputs = wordCloud(corpus, True)

        elif algorithm.lower() == "zipf law":
                outputs = zipfAll(corpus, True)
